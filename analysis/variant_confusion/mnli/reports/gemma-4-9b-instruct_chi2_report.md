...
| **paraphrase right**   | b = 187 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 111 (cases where the original itself was wrong
...
- 95% bootstrap CI = [-24, 49]
- bootstrap p-value = 0.51620 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.52766 >= 0.05). Although the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=187 vs c=174), with only 361 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-24, 49] (includes zero), and its empirical p-value is 0.51620. This is broadly consistent with the McNemar result above.
