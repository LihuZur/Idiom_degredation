...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 144 |
...
For reference, `original_only_wrong` = 110 (cases where the original itself was wrong
...
- observed diff (b - c) = 59
- 95% bootstrap CI = [22, 95]
- bootstrap p-value = 0.00280 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.00185 < 0.05): the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=203 vs c=144). This supports the research hypothesis that idiomatic phrasing degrades accuracy more than plain paraphrasing.

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [22, 95], which excludes zero, and its empirical p-value (0.00280) is also below alpha=0.05.
