...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 173 |
| **paraphrase right**   | b = 184 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 115 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 184
- c (paraphrase-only-wrong) = 173
- n = b + c = 357
- chi2 = 0.280, df = 1
- p-value = 0.59663
...
- 95% bootstrap CI = [-26, 48]
- bootstrap p-value = 0.57340 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.59663 >= 0.05). Although the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=184 vs c=173), with only 357 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-26, 48] (includes zero), and its empirical p-value is 0.57340. This is broadly consistent with the McNemar result above.
