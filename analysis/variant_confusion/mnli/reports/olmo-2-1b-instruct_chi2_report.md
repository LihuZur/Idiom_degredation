...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 187 |
| **paraphrase right**   | b = 228 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 118 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 228
- c (paraphrase-only-wrong) = 187
- n = b + c = 415
- chi2 = 3.855, df = 1
- p-value = 0.04959
...
- observed diff (b - c) = 41
- 95% bootstrap CI = [2, 81]
- bootstrap p-value = 0.04540 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.04959 < 0.05): the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=228 vs c=187). This supports the research hypothesis that idiomatic phrasing degrades accuracy more than plain paraphrasing.

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [2, 81], which excludes zero, and its empirical p-value (0.04540) is also below alpha=0.05.
