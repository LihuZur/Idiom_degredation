# McNemar significance report — granite-3.1-2b-instruct

## What is being compared

Restricting to the 4709 items where this model answered
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 137 |
| **paraphrase right**   | b = 154 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 92 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 154
- c (paraphrase-only-wrong) = 137
- n = b + c = 291
- chi2 = 0.880, df = 1
- p-value = 0.34828
...
- observed diff (b - c) = 17
- 95% bootstrap CI = [-17, 51]
- bootstrap p-value = 0.34220 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.34828 >= 0.05). Although the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=154 vs c=137), with only 291 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-17, 51] (includes zero), and its empirical p-value is 0.34220. This is broadly consistent with the McNemar result above.
