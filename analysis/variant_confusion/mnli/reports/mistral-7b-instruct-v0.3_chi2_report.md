...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 191 |
| **paraphrase right**   | b = 225 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 152 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 225
- c (paraphrase-only-wrong) = 191
- n = b + c = 416
- chi2 = 2.618, df = 1
- p-value = 0.10567
...
- observed diff (b - c) = 34
- 95% bootstrap CI = [-6, 74]
- bootstrap p-value = 0.10000 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.10567 >= 0.05). Although the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=225 vs c=191), with only 416 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-6, 74] (includes zero), and its empirical p-value is 0.10000. This is broadly consistent with the McNemar result above.
