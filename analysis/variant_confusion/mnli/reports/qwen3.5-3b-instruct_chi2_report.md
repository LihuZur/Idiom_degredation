# McNemar significance report — qwen3.5-3b-instruct
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 174 |
| **paraphrase right**   | b = 198 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 123 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 198
- c (paraphrase-only-wrong) = 174
- n = b + c = 372
- chi2 = 1.422, df = 1
- p-value = 0.23307
...
- observed diff (b - c) = 24
- 95% bootstrap CI = [-13, 63]
- bootstrap p-value = 0.21820 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.23307 >= 0.05). Although the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=198 vs c=174), with only 372 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-13, 63] (includes zero), and its empirical p-value is 0.21820. This is broadly consistent with the McNemar result above.
