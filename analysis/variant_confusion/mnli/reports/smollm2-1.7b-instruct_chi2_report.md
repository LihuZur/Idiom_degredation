# McNemar significance report — smollm2-1.7b-instruct
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 126 |
| **paraphrase right**   | b = 118 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 77 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 118
- c (paraphrase-only-wrong) = 126
- n = b + c = 244
- chi2 = 0.201, df = 1
- p-value = 0.65406
...
- observed diff (b - c) = -8
- 95% bootstrap CI = [-38, 23]
- bootstrap p-value = 0.61980 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.65406 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=126 vs b=118), with only 244 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-38, 23] (includes zero), and its empirical p-value is 0.61980. This is broadly consistent with the McNemar result above.
