...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 163 |
| **paraphrase right**   | b = 233 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 133 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 233
- c (paraphrase-only-wrong) = 163
- n = b + c = 396
- chi2 = 12.023, df = 1
- p-value = 0.00053
...
- 95% bootstrap CI = [31, 109]
- bootstrap p-value = 0.00060 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.00053 < 0.05): the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=233 vs c=163). This supports the research hypothesis that idiomatic phrasing degrades accuracy more than plain paraphrasing.

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [31, 109], which excludes zero, and its empirical p-value (0.00060) is also below alpha=0.05.
