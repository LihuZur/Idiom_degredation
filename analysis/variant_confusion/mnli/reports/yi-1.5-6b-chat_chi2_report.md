...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 138 |
| **paraphrase right**   | b = 194 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 94 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 194
- c (paraphrase-only-wrong) = 138
- n = b + c = 332
- chi2 = 9.111, df = 1
- p-value = 0.00254
...
- observed diff (b - c) = 56
- 95% bootstrap CI = [20, 91]
- bootstrap p-value = 0.00220 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.00254 < 0.05): the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=194 vs c=138). This supports the research hypothesis that idiomatic phrasing degrades accuracy more than plain paraphrasing.

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [20, 91], which excludes zero, and its empirical p-value (0.00220) is also below alpha=0.05.
