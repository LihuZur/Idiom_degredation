# McNemar significance report — h2o-danube3-4b-chat
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 172 |
| **paraphrase right**   | b = 164 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 127 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 164
- c (paraphrase-only-wrong) = 172
- n = b + c = 336
- chi2 = 0.146, df = 1
- p-value = 0.70255
...
- observed diff (b - c) = -8
- 95% bootstrap CI = [-44, 28]
- bootstrap p-value = 0.67800 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.70255 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=172 vs b=164), with only 336 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-44, 28] (includes zero), and its empirical p-value is 0.67800. This is broadly consistent with the McNemar result above.
