...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 154 |
| **paraphrase right**   | b = 224 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 99 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 224
- c (paraphrase-only-wrong) = 154
- n = b + c = 378
- chi2 = 12.595, df = 1
- p-value = 0.00039
...
- 95% bootstrap CI = [33, 108]
- bootstrap p-value = 0.00020 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.00039 < 0.05): the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=224 vs c=154). This supports the research hypothesis that idiomatic phrasing degrades accuracy more than plain paraphrasing.

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [33, 108], which excludes zero, and its empirical p-value (0.00020) is also below alpha=0.05.
