# McNemar significance report — falcon3-3b-instruct
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 170 |
| **paraphrase right**   | b = 240 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 149 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 240
- c (paraphrase-only-wrong) = 170
- n = b + c = 410
- chi2 = 11.612, df = 1
- p-value = 0.00066
...
- observed diff (b - c) = 70
- 95% bootstrap CI = [30, 108]
- bootstrap p-value = 0.00060 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.00066 < 0.05): the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=240 vs c=170). This supports the research hypothesis that idiomatic phrasing degrades accuracy more than plain paraphrasing.

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [30, 108], which excludes zero, and its empirical p-value (0.00060) is also below alpha=0.05.
