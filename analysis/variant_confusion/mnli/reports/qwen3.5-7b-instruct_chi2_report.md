# McNemar significance report — qwen3.5-7b-instruct
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 175 |
...
- observed diff (b - c) = 49
- 95% bootstrap CI = [10, 87]
- bootstrap p-value = 0.01480 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.01626 < 0.05): the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=224 vs c=175). This supports the research hypothesis that idiomatic phrasing degrades accuracy more than plain paraphrasing.

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [10, 87], which excludes zero, and its empirical p-value (0.01480) is also below alpha=0.05.
