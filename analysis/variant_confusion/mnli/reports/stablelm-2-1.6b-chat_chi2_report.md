# McNemar significance report — stablelm-2-1.6b-chat
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 157 |
| **paraphrase right**   | b = 162 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 109 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 162
- c (paraphrase-only-wrong) = 157
- n = b + c = 319
- chi2 = 0.050, df = 1
- p-value = 0.82279
...
- observed diff (b - c) = 5
- 95% bootstrap CI = [-30, 41]
- bootstrap p-value = 0.81400 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.82279 >= 0.05). Although the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=162 vs c=157), with only 319 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-30, 41] (includes zero), and its empirical p-value is 0.81400. This is broadly consistent with the McNemar result above.
