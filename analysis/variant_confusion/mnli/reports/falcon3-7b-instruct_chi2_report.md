# McNemar significance report — falcon3-7b-instruct
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 159 |
| **paraphrase right**   | b = 210 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 91 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 210
- c (paraphrase-only-wrong) = 159
- n = b + c = 369
- chi2 = 6.775, df = 1
- p-value = 0.00924
...
- observed diff (b - c) = 51
- 95% bootstrap CI = [14, 88]
- bootstrap p-value = 0.00880 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.00924 < 0.05): the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=210 vs c=159). This supports the research hypothesis that idiomatic phrasing degrades accuracy more than plain paraphrasing.

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [14, 88], which excludes zero, and its empirical p-value (0.00880) is also below alpha=0.05.
