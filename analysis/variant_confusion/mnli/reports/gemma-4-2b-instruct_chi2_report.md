# McNemar significance report — gemma-4-2b-instruct
...
| **paraphrase right**   | b = 203 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 148 (cases where the original itself was wrong
...
- observed diff (b - c) = 46
- 95% bootstrap CI = [9, 82]
- bootstrap p-value = 0.01840 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.01771 < 0.05): the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=203 vs c=157). This supports the research hypothesis that idiomatic phrasing degrades accuracy more than plain paraphrasing.

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [9, 82], which excludes zero, and its empirical p-value (0.01840) is also below alpha=0.05.
