...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 51 |
| **paraphrase right**   | b = 54 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 53 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 54
- c (paraphrase-only-wrong) = 51
- n = b + c = 105
- chi2 = 0.038, df = 1
- p-value = 0.84525
...
- observed diff (b - c) = 3
- 95% bootstrap CI = [-17, 23]
- bootstrap p-value = 0.80920 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.84525 >= 0.05). Although the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=54 vs c=51), with only 105 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-17, 23] (includes zero), and its empirical p-value is 0.80920. This is broadly consistent with the McNemar result above.
