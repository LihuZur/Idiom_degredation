...
| **paraphrase right**   | b = 22 | (both fine — excluded from the test) |
...
- observed diff (b - c) = -26
- 95% bootstrap CI = [-43, -10]
- bootstrap p-value = 0.00140 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.00281 < 0.05): the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=48 vs b=22). This is the OPPOSITE of the research hypothesis (paraphrase degraded accuracy more than idiomatic here).

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-43, -10], which excludes zero, and its empirical p-value (0.00140) is also below alpha=0.05.
