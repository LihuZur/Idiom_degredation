...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 63 |
| **paraphrase right**   | b = 34 | (both fine — excluded from the test) |
...
- b (idiomatic-only-wrong) = 34
- c (paraphrase-only-wrong) = 63
- n = b + c = 97
- chi2 = 8.082, df = 1
- p-value = 0.00447
...
- observed diff (b - c) = -29
- 95% bootstrap CI = [-48, -10]
- bootstrap p-value = 0.00320 (n_boot=10000)

## Conclusion

**Statistically significant at alpha=0.05** (p=0.00447 < 0.05): the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=63 vs b=34). This is the OPPOSITE of the research hypothesis (paraphrase degraded accuracy more than idiomatic here).

The bootstrap check **agrees**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-48, -10], which excludes zero, and its empirical p-value (0.00320) is also below alpha=0.05.
