...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 56 |
...
For reference, `original_only_wrong` = 38 (cases where the original itself was wrong
...
- observed diff (b - c) = 3
- 95% bootstrap CI = [-18, 24]
- bootstrap p-value = 0.82320 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.85205 >= 0.05). Although the **idiomatic** rewrite broke a previously-correct answer more often than the **paraphrase** rewrite did (b=59 vs c=56), with only 115 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-18, 24] (includes zero), and its empirical p-value is 0.82320. This is broadly consistent with the McNemar result above.
