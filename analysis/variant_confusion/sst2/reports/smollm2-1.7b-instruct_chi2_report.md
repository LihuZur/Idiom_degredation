...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 53 |
| **paraphrase right**   | b = 40 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 46 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 40
- c (paraphrase-only-wrong) = 53
- n = b + c = 93
- chi2 = 1.548, df = 1
- p-value = 0.21337
...
- 95% bootstrap CI = [-32, 5]
- bootstrap p-value = 0.18520 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.21337 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=53 vs b=40), with only 93 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-32, 5] (includes zero), and its empirical p-value is 0.18520. This is broadly consistent with the McNemar result above.
