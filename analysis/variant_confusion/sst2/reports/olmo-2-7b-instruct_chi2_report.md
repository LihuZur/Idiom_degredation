# McNemar significance report — olmo-2-7b-instruct
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 48 |
| **paraphrase right**   | b = 33 | (both fine — excluded from the test) |
...
- b (idiomatic-only-wrong) = 33
- c (paraphrase-only-wrong) = 48
- n = b + c = 81
- chi2 = 2.420, df = 1
- p-value = 0.11981
...
- observed diff (b - c) = -15
- 95% bootstrap CI = [-33, 3]
- bootstrap p-value = 0.11980 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.11981 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=48 vs b=33), with only 81 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-33, 3] (includes zero), and its empirical p-value is 0.11980. This is broadly consistent with the McNemar result above.
