# McNemar significance report — olmo-2-1b-instruct
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 57 |
| **paraphrase right**   | b = 44 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 44 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 44
- c (paraphrase-only-wrong) = 57
- n = b + c = 101
- chi2 = 1.426, df = 1
- p-value = 0.23246
...
- 95% bootstrap CI = [-32, 6]
- bootstrap p-value = 0.21100 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.23246 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=57 vs b=44), with only 101 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-32, 6] (includes zero), and its empirical p-value is 0.21100. This is broadly consistent with the McNemar result above.
