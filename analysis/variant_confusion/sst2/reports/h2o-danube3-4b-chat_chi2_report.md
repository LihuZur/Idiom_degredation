...
For reference, `original_only_wrong` = 28 (cases where the original itself was wrong
...
- observed diff (b - c) = -9
- 95% bootstrap CI = [-27, 9]
- bootstrap p-value = 0.35740 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.38555 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=47 vs b=38), with only 85 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-27, 9] (includes zero), and its empirical p-value is 0.35740. This is broadly consistent with the McNemar result above.
