...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 40 |
| **paraphrase right**   | b = 26 | (both fine — excluded from the test) |
...
- b (idiomatic-only-wrong) = 26
- c (paraphrase-only-wrong) = 40
- n = b + c = 66
- chi2 = 2.561, df = 1
- p-value = 0.10956
...
- 95% bootstrap CI = [-29, 2]
- bootstrap p-value = 0.09600 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.10956 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=40 vs b=26), with only 66 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-29, 2] (includes zero), and its empirical p-value is 0.09600. This is broadly consistent with the McNemar result above.
