# McNemar significance report — yi-1.5-6b-chat
...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 49 |
| **paraphrase right**   | b = 39 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 25 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 39
- c (paraphrase-only-wrong) = 49
- n = b + c = 88
- chi2 = 0.920, df = 1
- p-value = 0.33736
...
- observed diff (b - c) = -10
- 95% bootstrap CI = [-29, 8]
- bootstrap p-value = 0.30840 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.33736 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=49 vs b=39), with only 88 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-29, 8] (includes zero), and its empirical p-value is 0.30840. This is broadly consistent with the McNemar result above.
