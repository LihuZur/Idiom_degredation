...
| **paraphrase right**   | b = 45 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 19 (cases where the original itself was wrong
...
- observed diff (b - c) = -4
- 95% bootstrap CI = [-23, 15]
- bootstrap p-value = 0.73400 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.75700 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=49 vs b=45), with only 94 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-23, 15] (includes zero), and its empirical p-value is 0.73400. This is broadly consistent with the McNemar result above.
