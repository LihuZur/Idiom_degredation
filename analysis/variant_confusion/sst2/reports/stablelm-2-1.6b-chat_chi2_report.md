...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 75 |
| **paraphrase right**   | b = 61 | (both fine — excluded from the test) |
...
For reference, `original_only_wrong` = 72 (cases where the original itself was wrong
...
- b (idiomatic-only-wrong) = 61
- c (paraphrase-only-wrong) = 75
- n = b + c = 136
- chi2 = 1.243, df = 1
- p-value = 0.26496
...
- observed diff (b - c) = -14
- 95% bootstrap CI = [-37, 9]
- bootstrap p-value = 0.25420 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.26496 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=75 vs b=61), with only 136 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-37, 9] (includes zero), and its empirical p-value is 0.25420. This is broadly consistent with the McNemar result above.
