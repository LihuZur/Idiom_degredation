...
|------------------------|------------------|------------------|
| **paraphrase WRONG**   | (both broke it — excluded from the test) | c = 35 |
...
- observed diff (b - c) = -2
- 95% bootstrap CI = [-18, 14]
- bootstrap p-value = 0.86600 (n_boot=10000)

## Conclusion

**Not statistically significant at alpha=0.05** (p=0.90348 >= 0.05). Although the **paraphrase** rewrite broke a previously-correct answer more often than the **idiomatic** rewrite did (c=35 vs b=33), with only 68 discordant items this difference could plausibly be due to chance alone — no reliable conclusion should be drawn from this result in isolation.

The bootstrap check is **more cautious**: its 95% CI on (idiomatic_only minus paraphrase_only) is [-18, 14] (includes zero), and its empirical p-value is 0.86600. This is broadly consistent with the McNemar result above.
